"""
Configuration Management Module for Ontology

This module provides centralized configuration management for ontology operations,
supporting multiple configuration sources including environment variables, config files,
and programmatic configuration.

Supported Configuration Sources:
    - Environment variables: ONTOLOGY_BASE_URI, ONTOLOGY_REASONER, ONTOLOGY_FORMAT, etc.
    - Config files: YAML, JSON, TOML formats
    - Programmatic: Python API for setting ontology configurations
...
Key Features:
    - Environment variable support for ontology parameters
...
Main Classes:
    - OntologyConfig: Main configuration manager class for ontology module
...
Example Usage:
    ...
    >>> method_config = ontology_config.get_method_config("generate")
"""
...
class OntologyConfig:
            ...
            try:
                if config_file.endswith('.yaml') or config_file.endswith('.yml'):
                    ...
                    with open(config_file, 'r') as f:
                        ...
                        if "ontology_methods" in data:
                            self._method_configs.update(data["ontology_methods"])
    ...
    def _load_env_vars(self):
        env_mappings = {
            ...
            "ONTOLOGY_REASONER": ("reasoner", str),
            "ONTOLOGY_FORMAT": ("format", str),
            "ONTOLOGY_MIN_OCCURRENCES": ("min_occurrences", int),
            "ONTOLOGY_SIMILARITY_THRESHOLD": ("similarity_threshold", float),
