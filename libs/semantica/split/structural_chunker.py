...
class StructuralChunker:
    ...
    def _extract_structure(self, text: str) -> List[StructuralElement]:
        ...
        while i < len(lines):
            ...
            if line.startswith('#') or self._is_heading(line, lines, i):
                level = len(re.match(r'^#+', line).group()) if line.startswith('#') else 1
            ...
            # Regular paragraph
                ...
                para_lines = [line]
                j = i + 1
                while j < len(lines) and lines[j].strip() and not self._is_structure_element(lines[j]):
    ...
    def _group_elements(self, elements: List[StructuralElement], original_text: str) -> List[Chunk]:
        ...
        for element in elements:
            ...
            if current_size + element_size > self.max_chunk_size and current_chunk:
                ...
                end_pos = start_pos + len(chunk_text) if start_pos >= 0 else chunk_start + len(chunk_text)
            ...
            if self.respect_headers and element.type == "heading" and element.level <= 2 and current_chunk:
        ...
        if current_chunk:
            chunk_text = '\n\n'.join([e.text for e in current_chunk])
            start_pos = original_text.find(chunk_text[:50], chunk_start)
            end_pos = start_pos + len(chunk_text) if start_pos >= 0 else len(original_text)
